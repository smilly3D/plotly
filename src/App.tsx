...
import { airTemp, airTemp2 } from "./static/data";


function App() {
  // Calcula a média dos dados
const mean = airTemp.reduce((acc, val) => acc + val, 0) / airTemp.length;
const mean2 = airTemp2.reduce((acc, val) => acc + val, 0) / airTemp2.length;
  
  // Calcula o desvio padrão dos dados
const std = Math.sqrt(airTemp.reduce((acc, val) => acc + Math.pow(val - mean, 2), 0) / airTemp.length);
const std2 = Math.sqrt(airTemp2.reduce((acc, val) => acc + Math.pow(val - mean2, 2), 0) / airTemp2.length);
...
// Ordem dos dados
const sortedData = airTemp.slice().sort((a, b) => a - b);
const sortedData2 = airTemp2.slice().sort((a, b) => a - b);
...
const x = [];
const y = [];
for (let i = 0; i <= Math.max(...airTemp); i += 0.1) {
  x.push(i);
...
const histogram = {
  ...
  histnorm: '',
  opacity: 0.6,
  name: 'airTemp 1',  // Nome para a legenda
  marker: {
    color: 'blue'
  }
};
...
const histogram2 = {
  ...
  histnorm: '',
  opacity: 0.6,
  name: 'airTemp 2',  // Nome para a legenda
  marker: {
    color: 'red'
  }
};
